    ...
    protected function processParams(File $phpcsFile, $stackPtr, $commentStart)
    {
        ...
        foreach ($tokens[$commentStart]['comment_tags'] as $pos => $tag) {
            ...
            if ($tokens[($tag + 2)]['code'] === T_DOC_COMMENT_STRING) {
                ...
                $varSpace  = isset($matches[3]) ? strlen($matches[3]) : 0;
                $typeLen   = strlen($type);
                if ($typeLen > $maxType) {
                    $maxType = $typeLen;
                }
            ...
            $params[] = [
                ...
                'param_space'   => $tokens[($tag+1)]['length'],
        ...
        foreach ($params as $pos => $param) {
            ...
            // Check number of spaces after the @param tag.
            $spaces = 2;
            if ($param['param_space'] !== $spaces) {
                $error = 'Expected %s spaces after @param tag; %s found';
                $data  = [
                    $spaces,
                    $param['param_space'],
                ];

                $fix = $phpcsFile->addFixableError($error, $param['tag'], 'SpacingAfterParamTag', $data);
                if ($fix === true) {
                    $phpcsFile->fixer->beginChangeset();
            ...
            if ($param['type_space'] !== $spaces) {
                ...
                if ($fix === true) {
                    ...
                    // Doesn't duplicate comment when ParamCommentNewLine enabled
                    if (empty($param['commentLines'][0])) {
